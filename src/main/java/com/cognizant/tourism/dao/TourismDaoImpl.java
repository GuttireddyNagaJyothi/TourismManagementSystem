...
public class TourismDaoImpl implements TourismDAO {
	...
	@Override
	public Tourism searchTour(int id) {
		Session session=factory.getCurrentSession();
		Tourism tour=session.get(Tourism.class, id);
	...
	{
		String sql="from Tourism where id="+id;
		Session session=factory.getCurrentSession();
		Query<Tourism> query=session.createQuery(sql, Tourism.class);
		List<Tourism> tours=query.getResultList();
		if(tours.isEmpty())
			return false;
		return true;
	}
